...
def filter_subsample(file_name, subsampling_rate=16000,
                     overwrite = False,
    ...
    if os.path.isfile(target_file) and overwrite==False:
        ...
        with h5py.File(target_file, 'a') as write_file:
            ...
            # write_file['channels_dict'] = channels_dict
        return None
...
def main():
    ...
    target_folder = '/mnt/server_epl/SCN1A/R1648H_filt/'
    for i, file_name in enumerate(tqdm(lines, total=len(lines))):
        try:
            filter_subsample(file_name, subsampling_rate=1600,
                             overwrite = False,
